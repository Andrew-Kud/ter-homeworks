...
Ключ отключает финальное интерактивное подтверждение. Главная опасность — потерять контроль над изменениями из-за человеческой ошибки. Если ты находишься не в той ветке, не обновил стейт или в коде скрыты деструктивные изменения, Terraform молча их применил (например, удалит базу данных).

- Допустимо: повторное применение известных изменений, локальная разработка, ci/cd пайплайны.
Нужен в автоматизированных средах, где пауза ломает процесс, а человек в цепочке отсутствует. Некому нажать yes.
Планы предварительно сохраняют (-out=plan.tfplan) и валидируют, а -auto-approve лишь применяет уже проверенный файл.
При создании временных стендов для автотестов подтверждение не нужно, стенд всё равно умрет через 10 минут.
Утилиты для быстрого поднятия/сноса песочницы (make up, make down), где потеря данных не критична.
...
т.е по умолчанию при terraform destroy Terraform не только удаляет запись о ресурсе из своего state-файла, но и физически удаляет образ из локального Docker (выполняет docker rmi). Аргумент keep_locally = true отключает физическое удаление: Terraform забывает об образе в стейте, но сам файл образа остается в кэше Docker на хосте.
